...
def _enviar_documentos(pid: int, ixc_cliente_id: int, ixc_contrato_id: int, pasta):
    import os, requests as _req, sqlite3 as _sq
    ...
    razao_curta = (pre["razao"] or "").split()[0].upper() if pre else ""
    for doc in docs:
        ...
        try:
            with open(doc_path, "rb") as f_doc:
                r = _req.post(
                    ...
                    data={"id_cliente": str(ixc_cliente_id), "descricao": descricao},
                    timeout=15,
                )
        ...
        try:
            with open(pdf_path, "rb") as f_pdf:
                r = _req.post(
                    ...
                    data={"id_cliente": str(ixc_cliente_id), "descricao": descricao_pdf},
                    timeout=15,
                )
